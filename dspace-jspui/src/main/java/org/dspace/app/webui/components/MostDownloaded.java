package org.dspace.app.webui.components;

import org.apache.commons.lang.ArrayUtils;
import org.dspace.content.Item;

/**
 * Created by katepechekhonova on 2/1/16.
 */
public class MostDownloaded {

    /** The set of items being represented */
    private Item[] items;

    /**
     * Construct a new MostDownloaded object to represent the passed
     * array of items
     *
     * @param items
     */
    public MostDownloaded(Item[] items)
    {
        this.items = (Item[]) ArrayUtils.clone(items);
    }

    /**
     * obtain the number of most downloaded available
     *
     * @return	the number of items
     */
    public int count()
    {
        return items.length;
    }

    /**
     * Obtain the array of items
     *
     * @return	an array of items
     */
    public Item[] getMostDownloaded()
    {
        return (Item[])ArrayUtils.clone(items);
    }

    /**
     * Get the item which is in the i'th position.  Therefore i = 1 gets the
     * most downloaded item, while i = 3 gets the 3rd most downloaded item
     *
     * @param i		the position of the item to retrieve
     * @return		the Item
     */
    public Item getMostDownloaded(int i) {
        if (i < items.length) {
            return items[i];
        } else {
            return null;
        }
    }
}
