...
import org.dspace.submit.step.RightsStep;
 ...
 * @see org.dspace.submit.step.RightsStep
 *
 * @author Tim Donohue
 * @version $Revision$
 */
public class JSPRightsStep extends JSPStep
{
    /** JSP which displays HTML for this Class * */
    private static final String DISPLAY_JSP = "/submit/edit-rights.jsp";
    
    /** JSP which reviews information gathered by DISPLAY_JSP * */
    private static final String REVIEW_JSP = "/submit/review-rights.jsp";

    /** form definition file for Rights metadata * */
    private static final String RIGHTS_FILE_NAME = "input-rights";
    private static final String RIGHTS_FILE_TYPE = ".xml";

    /** log4j logger */
    private static Logger log = Logger.getLogger(JSPRightsStep.class);

    /** Constructor */
    public JSPRightsStep() throws ServletException
    {
        super();
    }
            ...
            AuthorizeException
    {
        ...
        String buttonPressed = UIUtil.getSubmitButton(request, RightsStep.NEXT_BUTTON);
        ...
        if (status == RightsStep.STATUS_MORE_INPUT_REQUESTED)
        {
        ...
        else if (status == RightsStep.STATUS_MISSING_REQUIRED_FIELDS)
        {
            List<String> missingFields = RightsStep.getErrorFields(request);
    ...
    {
        String formFileName = getRightsFormsFilePath(RIGHTS_FILE_NAME, RIGHTS_FILE_TYPE);
        ...
        try
        {
            request.setAttribute("submission.inputs", RightsStep.getInputsReader(formFileName).getInputs(c
                    .getHandle()));
        }
    ...
    /**
     * Get the Rights Input Forms file location
     * Note: this method does not support Locales at this time.
     *
     * @return String - localized filename for input-forms.xml
     */
    private static String getRightsFormsFilePath(String fileName, String fileType)
    {
        /** Name of the form definition XML file */
        String filePath = ConfigurationManager.getProperty("dspace.dir")
	    + File.separator + "config" + File.separator + fileName + fileType;
        return filePath;
    }
}
